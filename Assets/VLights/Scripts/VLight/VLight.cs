...
public class VLight : MonoBehaviour
{
	...
	public MaterialPropertyBlock CreatePropertiesBlock()
	{
		...
		material.AddVector(_idLightParams, new Vector4(near, far, far - near, (GetComponent<Camera>().orthographic) ? Mathf.PI : fov * 0.5f * Mathf.Deg2Rad));
	...
	public void SetShaderPropertiesBlock(MaterialPropertyBlock propertyBlock)
	{
		...
		propertyBlock.AddVector(_idLightParams, new Vector4(near, far, far - near, (GetComponent<Camera>().orthographic) ? Mathf.PI : fov * 0.5f * Mathf.Deg2Rad));
	}
